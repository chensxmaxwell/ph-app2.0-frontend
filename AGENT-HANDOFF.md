...
| `src/services/cloud-tts.ts` | Doubao V3 HTTP TTS: `buildTtsRequest` (`X-Api-Key` or `X-Api-App-Id` + `X-Api-Access-Key`, `X-Api-Resource-Id` from the speaker family), `parseTtsStream` (concatenated JSON or SSE, base64 `data` pieces, close code 20000000), `splitForSynthesis` (≤ 220-char sentence runs), `synthesizeSpeech` → `{ ok, chunks }` or `{ reason: auth / voice / request_failed }` |
| `src/services/speech-engine.ts` | `createSpeechEngine(deps)`: cloud first (`playAudio` chunks), on-device voice of the same gender + detected language second; a refused key turns the cloud off for the session, a refused speaker only for that voice; `stop` aborts a synthesis in flight |
...
6. Voice follow-ups, all documented for the 2.0 speakers we use and all **untested on the phone**, so they were left out of the request shape on purpose: `additions.context_texts` (one free-text 语音指令 such as "像打电话一样自然、亲密地说", not billed — the expressive tier is exactly the tier that follows it), `additions.section_id` (one UUID per call so Doubao keeps prosody across the turns of that call), streaming Doubao audio (V3 WebSocket) instead of one HTTP round trip per sentence run, and a voice picker on the Identity page (the catalogue in `voices.ts` is ready, tiers included). `req_params.model = seed-tts-2.0-expressive` is documented for voice cloning only — do not send it for stock speakers.
