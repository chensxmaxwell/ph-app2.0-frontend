...
| Voice / video call | **Real, on-device loop, push-to-talk.** Phone icon on a Message thread or Love chat → `src/screens/call/` (`useVoiceCall`): hold → PHNative iOS Speech (`startVoiceInput` / `stopVoiceInput`) → `completeCompanionChat` (Ark, that person's name / personality / story / transcript) → PHNative `AVSpeechSynthesizer` speaks the reply while the face pulses. Video = same call with the companion's `companionFace` on the stage (3D `AvatarPreview` bust if they have a look, else the portrait) and the user's front camera as a PiP (`PHCameraPreview`, native `AVCaptureSession`, video only). No WebRTC, no third-party STT/TTS/video vendor, no VAD (it would fight `AVAudioSession` with the recognizer and the avatar WKWebView). Not device-verified as of this entry (Jest only). |
