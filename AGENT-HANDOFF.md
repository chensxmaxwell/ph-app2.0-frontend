...
| File | What |
|---|---|
| `src/screens/call/use-voice-call.ts` | The hands-free loop. `phase`: `connecting` → `ready` → (opener: `speaking`) → `listening` (`listenForUtterance`, one utterance at a time) → `thinking` (`completeCompanionChat`, history = chat + `transcript`) → `speaking` (`ttsSpeak` resolves when the utterance ends) → `listening` again. On connect (`connectDelayMs` > 0) `greetThenListen`: Ark opener with `OPENER_INSTRUCTION` when a key is saved, else `localOpener(name)`; `connectDelayMs` 0 (restored / re-entered) = `ttsStop()` then listen, no opener. `pressMic`: speaking → barge-in (`ttsStop`, listen); listening → mute (`stopVoiceInput`, `ready`); muted / ready / thinking → listen. Every user action bumps a turn token; an async step only applies its result if its token is still current. An empty listen is reopened silently; one that ends within `LISTEN_FAST_EMPTY_MS` of opening is reopened after `LISTEN_RETRY_DELAY_MS`, and `LISTEN_MAX_FAST_EMPTIES` in a row stop the loop with `LISTEN_UNRESPONSIVE_COPY`; a mic failure or `missing_key` stops it with its copy. `hangUp()` stops mic + voice and marks the hook dead; unmount without hang-up (minimize) closes the mic and stops the loop (a reply mid-sentence finishes). Mount: `hasLlmKey` → the chats' "Add a key" copy; `ttsCredentialsFromConfig` → `voiceHint` unless a speech-console key (`source: "tts"` or app id + token) is saved. `transcript` (`CallTurn[]`) is the call's own record and is never written anywhere. `CALL_CONNECT_DELAY_MS` 1600, `LISTEN_SILENCE_MS` 1100, `LISTEN_MAX_MS` 20000, `LISTEN_IDLE_MS` 45000 |
...
| `src/screens/call/status.ts` | `CallPhase`, `callStatusLabel` (`Calling Kevin` / `Connected` / `Listening…` / `Thinking…` / `Kevin is speaking`), `micButtonLabel({ phase, muted })` (`Connecting…` / `Listening` / `Thinking…` / `Tap to interrupt` / `Tap to talk` / `Muted` — never hold-to-talk), `voiceKeyHint(name)`, `modeToggle(video)` → `{ target, label }` (the third control names the mode it switches **to**: camera glyph + "Video" on a voice call, Figma handset + "Voice" on a video call — icon and label always agree) |
| `src/screens/call/call-body.tsx` | Everything on a call but the backdrop: minimize (`call-minimize`), name + timer, audio stage (`LookFace` in a ring that pulses while speaking, `call-stage-face`) or `VideoStage`, captions (`call-status`, `call-heard`, `call-reply`, `call-notice`, `call-voice-hint`), controls row = mic tap (`call-mic`, hot while listening, muted style + `microphone-mute.svg` when muted, disabled until connected; **no `Pressable`, no `onPressIn`**) / hang-up (`call-hangup`) / video toggle (`call-video-toggle`) |
