...
| `src/screens/avatar/identity.tsx` … `intimate.tsx` `candle.tsx` `waiting.tsx` | Steps. `identity.tsx` is the **basic info page** (name, gender, birthday, description). `waiting.tsx` saves companion then `CommonActions.reset` to `NAV_BAR → CHAT_THREAD { threadId }` (`routesAfterCompanionSaved`) — the person's real Message thread with Home underneath; it does **not** start a Love session or open `LOVE_CHAT` |
| `src/screens/avatar/face.ts` | `CompanionFace` (`kind`, `look`, `source`) — the one face a person shows everywhere. `companionFace({ thread, companion, id, kind, choice })`: crafted look wins unless the thread's `avatar` pick is `portrait` **and** the person owns a photo; no look → photo → stock. `avatarOptions` lists what can be picked. Render with `<LookFace look={face.look} fallbackSource={face.source} />` |
...
| `src/screens/avatar/avatar-picker.tsx` | Row of pickable faces (`avatar-option-look` / `avatar-option-portrait`), ringed selection |
| `src/screens/avatar/use-save-companion.ts` | The one save path. `create` folds a seeded name onto the seed id (a new 3D "Kevin" is record **and** thread `kevin`); create / editLook set the thread's `avatar` pick back to `look`; editPersona on a chat-only bot writes the thread only |
...
| `assets/avatar-engine/viewer-page.html` internals | `figureMeshVisible` (which Body_/Head_/Eyes_/Hair_/Outfit_ meshes a look draws, `SKIN_BY_OUTFIT` = skin each outfit leaves bare), `retargetSkeletons` (23 armatures → one master skeleton by glTF bone name), `placeFigure`/`measureFigureBox` (skinned bbox via `boneTransform`), `OUTFIT_POSES` + `poseArm` (two-bone IK: wrist target + elbow pole per side, metres from the shoulder joint), `IRIS_RADIUS` / `PUPIL_RADIUS` + `irisFragmentChunk` (the eyeball GLSL; radii in the shader's normalised units where the lid opening is ~0.69 wide / ~0.39 tall from the iris centre, scaled by `irisSize` 0.82..1.10 from the Eyes slider). `window.phViewerState()` and `window.phViewerRig` are read-only probes for tests. Metro cache-bust query lives in `AvatarEngineHost.tsx` (`?v=bozo27`); bump it with every viewer edit |
| `scripts/check-avatar-viewer.js` | Renders the real viewer in headless Chrome and asserts figure/hands in frame, eyeballs in the head, hair on the head, one master rig; writes screenshots. `node scripts/check-avatar-viewer.js --out /tmp/ph-avatar-check` |
...
| Companion face | One face per person on Home, Message list, thread header/hero, Chat settings, Love chat header, Love pill, Love call, Love/Control Sync, Sync picker (`companionFace`). Crafted look by default; **avatar picker** in Chat settings (thread lightbulb) and Love `···` ("Use photo" / "Use 3D avatar") switches to the seeded photo and back, persisted on the thread (`ph.chat.v2` `avatar`). Jest-verified in `__tests__/companion-avatar-identity.test.tsx`; not device-verified as of this entry. |
