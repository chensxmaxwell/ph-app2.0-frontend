...
| Voice / video call | **Real, on-device loop, push-to-talk.** Phone icon on a Message thread or Love chat → `src/screens/call/` (`useVoiceCall`): hold → PHNative iOS Speech (`startVoiceInput` / `stopVoiceInput`) → `completeCompanionChat` (Ark, that person's name / personality / story / transcript) → the person's own voice speaks the reply while the face pulses (Doubao Seed-TTS 2.0 through `PHNative.playAudio`, else a gendered `AVSpeechSynthesizer` voice — see **Companion voice**). Video = same call with the companion's `companionFace` on the stage (3D bust through `InlineAvatarViewer` if they have a look, else the portrait filling the stage) and the user's front camera as a PiP (`PHCameraPreview`, native `AVCaptureSession`, video only, copy for every non-live state). No WebRTC, no third-party STT/TTS/video vendor, no VAD (it would fight `AVAudioSession` with the recognizer and the avatar WKWebView). Not device-verified as of this entry (Jest only). |
| Companion voice | **Every companion speaks in a voice of their gender.** One Doubao Seed-TTS 2.0 speaker per person (`src/services/voices.ts`: 8 female `zh_female_*_uranus_bigtts`, 6 male `zh_male_*_uranus_bigtts`), drawn **at random from the gender's pool when the character is created** and persisted on the companion record and the thread (`voiceId`); Non-binary draws from both pools. Seeds: Amanda 小何 (female), Kevin 云舟, Chad 儒雅逸辰 (male). Calls and Listen speak with it. Cloud = 豆包语音 V3 TTS (needs the **speech console's own API key** — the Ark key is not accepted there; Profile → Companion AI → Voice key, or `TTS_API_KEY`); fallback = AVSpeechSynthesizer voice picked natively by gender + language. Not device-verified as of this entry. |
...
| `src/screens/love/call.tsx` | Voice / video call overlay on the Love session: `resolveLovePerson` → `useVoiceCall` + `CallBody` (see `src/screens/call/`). Spoken turns are appended to `chat.messages` through `patchChat` so minimize keeps them; `connectDelayMs` is 0 when `callStartedAt` is already set (pill restore does not ring again). Minimize = pill; hangup = `call.hangUp()`, `inCall: false`, clear timer, back to chat |
...
| `src/screens/chat/call.tsx` | Voice / video call on a Message thread: `useVoiceCall` grounded in `thread.messages` / `personality` / `description`, `CallBody` over `ChatGradient`. Each spoken turn is written to the thread by `recordCallExchange` (store; never `replyTo`, which would read it aloud again). Minimize keeps `inCall`; hangup clears it (the `endedRef` split, landmine 6) |
...
| File | What |
|---|---|
| `src/screens/call/use-voice-call.ts` | The loop. `phase`: `connecting` → `ready` → `listening` (hold) → `thinking` (`stopVoiceInput` transcript → `completeCompanionChat`) → `speaking` (`ttsSpeak` resolves when the utterance ends) → `ready`. Every user action bumps a turn token; an async step only applies its result if its token is still current, so barge-in (hold while speaking = `ttsStop`) and hang-up cancel in-flight work. `hangUp()` stops mic + voice and marks the hook dead; unmount without hang-up (minimize) only releases the mic — a reply already on its way still lands in the chat and is spoken. Checks `hasLlmKey` on mount and shows the same "Add a key in Companion AI settings" copy the chats use (`companionChatFailureMessage("missing_key")`). `CALL_CONNECT_DELAY_MS` = 1600 |
| `src/screens/call/status.ts` | `CallPhase`, `callStatusLabel` (`Calling Kevin` / `Connected` / `Listening…` / `Thinking…` / `Kevin is speaking`), `holdButtonLabel`, `NOTHING_HEARD_COPY`, `modeToggle(video)` → `{ target, label }` (the third control names the mode it switches **to**: camera glyph + "Video" on a voice call, Figma handset + "Voice" on a video call — icon and label always agree) |
| `src/screens/call/call-body.tsx` | Everything on a call but the backdrop: minimize (`call-minimize`), name + timer, audio stage (`LookFace` in a ring that pulses while speaking, `call-stage-face`) or `VideoStage`, captions (`call-status`, `call-heard`, `call-reply`, `call-notice`), controls row = hold-to-talk `Pressable` (`call-hold`, disabled until connected) / hang-up (`call-hangup`) / video toggle (`call-video-toggle`) |
...
| `ios/AppFrontend/PHNative.mm` | `PHCameraPreviewManager` / `PHCameraPreviewView` (`RCT_EXPORT_MODULE(PHCameraPreview)`, props `position`, `onStatusChange` with `authorized` / `running` / `interrupted` / `denied` / `unavailable`): `AVCaptureSession` with the front wide camera only, `automaticallyConfiguresApplicationAudioSession = NO`, starts in `didMoveToWindow`, observes `AVCaptureSessionDidStartRunning` / `RuntimeError` / `WasInterrupted` / `InterruptionEnded` and `UIApplicationDidBecomeActive` (back from Settings restarts it). `speak(text, options)` picks an `AVSpeechSynthesisVoice` by `gender` + `language` (`voiceForOptions`); `playAudio(chunks)` joins base64 MP3 pieces and plays them with `AVAudioPlayer`; `stopSpeaking` stops both. `ensurePlaybackAudioSession` runs before every speak / play and lifts any non-playback category (landmine 22). `NSCameraUsageDescription` is in both Info.plists (`__tests__/ios-privacy-plist.test.ts`). **The native side has been read and reasoned about, never compiled here** |

`__tests__/voice-video-call.test.tsx` drives both call screens with speech / Ark / TTS mocked at the service edge (stage layout, inline viewer, PiP states, toggle, voices); `__tests__/camera-preview-gate.test.tsx` locks the UIManager gate; `__tests__/cloud-tts.test.ts` the Doubao request / response contract and the speech engine; `__tests__/companion-voices.test.tsx` the voice pools, seeds and the create-wizard assignment.
...
| `src/services/voices.ts` | The voice catalogue and rules: `FEMALE_VOICES` / `MALE_VOICES` (Doubao Seed-TTS 2.0 speakers), `SEED_VOICES`, `assignVoiceForGender(gender, random)`, `voiceIdForGender(current, gender)` (keep while it fits, else redraw), `voiceForPerson({ thread, companion, id })` (thread pick → record pick → seed → stable hash draw from the gender pool for blobs from before voices existed) |
...
| Voice / video call | Real push-to-talk conversation (`src/screens/call/`): hold → iOS Speech → Ark → the person's voice (Doubao Seed-TTS 2.0, else a gendered AVSpeech voice), spoken turns written to the Message thread / Love transcript, video mode = this person's face (3D bust through the in-place viewer, or the portrait filling the stage) + front-camera PiP with copy for every non-live state. Same missing-key copy as the chats. **No WebRTC** (nothing to call; the companion runs on the phone). TestFlight 1.2 (14) showed Amanda's portrait at its own size in the stage corner, an empty PiP and a camera glyph labelled Voice — fixed in Jest; **not device-verified as of this entry.** |
...
22. **A call is one shared `AVAudioSession` plus one WKWebView; keep every step off each other's toes.** (a) PHNative's speech recognizer sets the shared session to `Record` / `Measurement` and `teardownVoiceEngine` only deactivates it — the category stays `Record`, which has no output route, so the very next `AVSpeechSynthesizer` utterance is silent. `speak` now calls `ensurePlaybackAudioSession` (move `Record` → `Playback` + `DuckOthers`, `setActive:YES`) before speaking; do not route the reply through anything that skips `speak`. (b) That is also why the call is push-to-talk and not VAD: a recognizer that listens while the synthesizer speaks would need the session in `PlayAndRecord` with echo cancellation and would hear the companion; hold → release → speak is sequential on purpose. (c) The front-camera PiP is a separate `AVCaptureSession` with video input only and `automaticallyConfiguresApplicationAudioSession = NO`; never add an audio input to it. (d) `requireNativeComponent("PHCameraPreview")` on a binary that never registered the view (Android, an IPA built before the view manager) throws at render → RCTFatal in Release (landmine 11); `src/native/camera-preview.ts` asks `UIManager.getViewManagerConfig` first and the screen shows copy instead. (e) The 3D avatar on the video stage is the app-wide engine WebView floated at the stage's measured rect, so the PiP and the controls live outside that rect (`VideoStage` puts the PiP in a row above the stage) or they render underneath the avatar. `__tests__/voice-video-call.test.tsx` + `__tests__/camera-preview-gate.test.tsx` lock the JS side; the native side has only been read, not built — the first TestFlight of this needs the Xcode build log if it fails to compile. (f) TestFlight 1.2 (14): the PiP was an empty dark box. The JS had copy only for `denied` / `unavailable` and the native view only ever said `authorized` (session configured), so a session that never delivered — or an event that never arrived — looked like a black rectangle. The view now reports `running` from `AVCaptureSessionDidStartRunningNotification` (the only state that clears the copy), `interrupted`, runtime errors as `unavailable` with the reason, and restarts on `UIApplicationDidBecomeActive`; the JS shows `Starting camera…`, `Camera paused…`, and after 6 s without frames `Camera didn't start.` + Retry. Whether frames actually flow on the phone is still unverified — if the copy stays on `Starting camera…`, the session never started and the reason is native (Console.app, `AVCaptureSession`).
...
4. Optional later: bundle GLB for offline phone; real LLM streaming; hands-free calls (VAD) only if the `AVAudioSession` split in landmine 22 is solved natively (`PlayAndRecord` + `VoiceChat` mode with the recognizer paused while the synthesizer speaks).
5. Voice follow-ups: a voice picker on the Identity page (the catalogue in `voices.ts` is ready for it); streaming Doubao audio (V3 WebSocket) instead of one HTTP round trip per sentence run; English-first speakers (`en_female_dacey_uranus_bigtts`, `en_male_tim_uranus_bigtts`) if replies are mostly English.
