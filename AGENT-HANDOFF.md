...
| Companion voice | **Every companion speaks in a voice of their gender.** One Doubao Seed-TTS 2.0 speaker per person (`src/services/voices.ts`: 10 female — 8 `zh_female_*` + Dacey, Stokie — and 7 male — 6 `zh_male_*` + Tim — all `*_uranus_bigtts` on `seed-tts-2.0`), drawn **at random from the gender's pool when the character is created** and persisted on the companion record and the thread (`voiceId`); Non-binary draws from both pools. Seeds (Maxwell talks to them in English): **Kevin Tim, Amanda Dacey (US English), Chad 刘飞**; a seeded thread still on a retired default (云舟 / 小何 / 儒雅逸辰) follows the new one on hydrate (`refreshedSeedVoiceId`). Calls and Listen speak with it. Cloud = 豆包语音 V3 TTS (needs the **speech console's own API key** — the Ark key is not accepted there; Profile → Companion AI → Voice key, or `TTS_API_KEY`); fallback = AVSpeechSynthesizer voice picked natively by gender + language, and **the call says so** (`voiceKeyHint`: "Using the phone's voice. Add a Voice key in Companion AI…") until a key is saved. Not device-verified as of this entry. |
...
| `src/services/voices.ts` | The voice catalogue and rules: `FEMALE_VOICES` / `MALE_VOICES` (Doubao Seed-TTS 2.0 speakers, Chinese + the three US-English ones), `SEED_VOICES` (Kevin Tim, Amanda Dacey, Chad 刘飞), `RETIRED_SEED_VOICES` + `refreshedSeedVoiceId(threadId, current)` (a seeded thread on an old default follows the new one; `mergeSeedThreads` applies it on hydrate), `assignVoiceForGender(gender, random)`, `voiceIdForGender(current, gender)` (keep while it fits, else redraw), `voiceForPerson({ thread, companion, id })` (thread pick → record pick → seed → stable hash draw from the gender pool for blobs from before voices existed) |
...
6. Voice follow-ups: a voice picker on the Identity page (the catalogue in `voices.ts` is ready for it, English speakers included); streaming Doubao audio (V3 WebSocket) instead of one HTTP round trip per sentence run; a per-call `additions.section_id` so Doubao keeps prosody across the turns of one call.
