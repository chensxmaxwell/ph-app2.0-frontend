...
- Cloud voice: `src/services/tts-config.ts` reads `@env` `TTS_API_KEY` (new 豆包语音 console, `X-Api-Key`) or `TTS_APP_ID` + `TTS_ACCESS_TOKEN` (legacy console); the saved Companion AI voice key wins. With none of those the Ark key is tried once and dropped for the session when refused (landmine 25). Static `import … from "@env"` only.
...
| `src/screens/love/call.tsx` | Voice / video call overlay on the Love session: `resolveLovePerson` → `useVoiceCall` + `CallBody` (see `src/screens/call/`). The Love chat grounds the replies; **nothing said on the call is appended to `chat.messages`** (landmine 26). `connectDelayMs` is 0 when `callStartedAt` is already set (pill restore does not ring or greet again; it cuts any voice still playing and listens at once). Minimize = pill; hangup = `call.hangUp()`, `inCall: false`, clear timer, back to chat |
...
| `src/screens/call/camera-preview.tsx` | `CameraPreview`: the native front camera with copy for every state but live video — `Starting camera…` until the native view reports `running`, `Camera paused by the system.` on `interrupted`, `Camera didn't start.` + Retry (remounts the view) after `CAMERA_START_TIMEOUT_MS` = 6 s without frames, `CAMERA_DENIED_COPY` + Open Settings on `denied`, the native message on `unavailable`, `CAMERA_UNAVAILABLE_COPY` when the binary has no view manager. A late `authorized` after `running` is ignored (never cover a live preview). Never an empty dark box |
| `src/native/camera-preview.ts` | `cameraPreviewAvailable()` (iOS and `UIManager.getViewManagerConfig("PHCameraPreview")` present) gates `requireNativeComponent`, which is created once. Never render the native view without this gate (landmine 22) |
...
| `ios/AppFrontend/PHNative.mm` | Module: `speak(text, options)` picks an `AVSpeechSynthesisVoice` by `gender` + `language` (`voiceForOptions`); `playAudio(chunks)` joins base64 MP3 pieces and plays them with `AVAudioPlayer`; `stopSpeaking` stops both; `ensurePlaybackAudioSession` runs before every speak / play and lifts any non-playback category (landmine 22). `startVoiceInput` / `stopVoiceInput` = push-to-talk. **`listenForUtterance(options)`** = hands-free: same recognizer (`Record` / `Measurement`, `addsPunctuation` on iOS 16+), a 150 ms main-queue tick ends the utterance when the transcript has not changed **and** the mic RMS stayed under `PHVoiceRmsFloor` (0.02) for `silenceMs`, or at `maxMs`; an empty listen ends as `idle` at `idleMs`; a recognizer error hands back what it had. `stopVoiceInput` / a newer start settle a pending listen as `stopped`; `voiceGeneration` keeps a listen superseded during the permission prompt from opening the mic. View: `PHCameraPreviewManager` / `PHCameraPreviewView` (`RCT_EXPORT_MODULE(PHCameraPreview)`, props `position`, `onStatusChange` with `authorized` / `running` / `interrupted` / `denied` / `unavailable`): **`+layerClass` = `AVCaptureVideoPreviewLayer`** (the layer is the view — no sublayer, no frame to copy), session created on main and handed to the layer, configured and started on `_sessionQueue` (front wide camera only, `automaticallyConfiguresApplicationAudioSession = NO`), **`running` from KVO on the layer's `previewing`** (frames actually on screen), `DidStopRunning` behind our back → `interrupted`, `RuntimeError` → `unavailable`, `WasInterrupted` / `InterruptionEnded`, `UIApplicationDidBecomeActive` (back from Settings restarts it). `NSCameraUsageDescription` is in both Info.plists (`__tests__/ios-privacy-plist.test.ts`). **The native side has been read and reasoned about, never compiled here** |

`__tests__/voice-video-call.test.tsx` drives both call screens with speech / Ark / TTS mocked at the service edge (opener, hands-free loop, barge-in, mute, idle / fast-empty backoff, hang-up, no chat writes, stage layout, inline viewer, PiP states, toggle, voices, native source locks); `__tests__/camera-preview-gate.test.tsx` locks the UIManager gate; `__tests__/voice-input.test.ts` the service edge; `__tests__/cloud-tts.test.ts` the Doubao request / response contract and the speech engine; `__tests__/companion-voices.test.tsx` the voice pools, seeds, the retired-default refresh and the create-wizard assignment; `__tests__/companion-llm.test.ts` the opener request shape.
...
| `src/services/TtsHost.tsx` | Configures the engine from `speech-engine.ts` (no WebView — landmine, see `voice-input.test.ts`) |
...
| `src/services/tts-config.ts` | `resolveTtsCredentials` / `ttsCredentialsFromConfig` (saved voice key → `TTS_API_KEY` → legacy app id + token → Ark key best effort) |
...
| Voice / video call | Real hands-free conversation (`src/screens/call/`): the companion greets first → the mic listens on its own (iOS Speech, native end-pointing) → Ark → the person's voice (Doubao Seed-TTS 2.0, else a gendered AVSpeech voice plus the "add a Voice key" hint) → listens again; tap the mic to interrupt or mute — never to talk (1.2 (18) still showed "Tap to talk" at 00:01: that was the `ready` window while the opener came back from Ark; it is now `greeting` with an inert `Connecting…` mic, the opener wait is bounded, and a stalled recognizer is reopened by the loop instead of stopping for a tap). **Nothing said on the call is written to the Message thread or the Love chat.** Video mode = this person's face (3D bust through the in-place viewer, or the portrait filling the stage) + front-camera PiP whose `running` means the preview layer is painting. Same missing-key copy as the chats. **No WebRTC** (nothing to call; the companion runs on the phone). TestFlight 1.2 (15) showed the loop working end to end (You / Kevin lines) but a black PiP with no copy, hold-to-talk, the phone's voice, and the call lines leaking into the chat — all addressed in Jest; **not device-verified as of this entry.** |
| Listen | Reads bubbles in the person's own voice (same engine as calls). Cloud needs the speech-console key; on-device fallback is gendered. |
