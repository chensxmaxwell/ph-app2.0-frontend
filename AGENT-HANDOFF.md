...
| `assets/avatar-engine/viewer-page.html` internals | `figureMeshVisible` (which Body_/Head_/Eyes_/Hair_/Outfit_ meshes a look draws, `SKIN_BY_OUTFIT` = skin each outfit leaves bare), `retargetSkeletons` (23 armatures → one master skeleton by glTF bone name), `placeFigure`/`measureFigureBox` (skinned bbox via `boneTransform`), `OUTFIT_POSES` + `poseArm` (two-bone IK: wrist target + elbow pole per side, metres from the shoulder joint), the eye block: `IRIS_RADIUS` 0.31 / `PUPIL_RADIUS` 0.115 + `irisFragmentChunk` (the eyeball GLSL; radii in the shader's normalised units where d = angle from the iris axis / 90° — measured exact on the GLB's UVs, so 0.31 is a 27.9° iris, the real iris-to-eyeball proportion; the rest lids uncover 0.26 above / 0.35 below / 0.40 nose / 0.66 temple), the **Eyes Size look model** — one slider, three couplings, all in `applyLook`: `eyeScaleFor` (`EYE_SCALE_MIN` 0.74 → `EYE_SCALE_MAX` 0.96, uniform scale on the two `eyeRoot` bones via `applyEyeScale`; `EYE_SCALE` 0.85 is the Size 0.5 default — #36's 0.58 read as too small on the craft walk, #40's 0.65 / 0.72 / 0.76 passes still too small on review, the 0.82 reference pass was the right direction and 0.85 is the nudge he asked for), `upperLidDrop` (Head_0 `Shape_EyeLidHeight` `UPPER_LID_DROP` 0.29 at Size 0 → `UPPER_LID_DROP_LARGE` 0.21 at Size 1, heavier for small eyes; the lid reaches the pupil at ~0.34) and `irisSizeFor` (`IRIS_SIZE_SMALL` 1.10 → `IRIS_SIZE_LARGE` 1.0, the painted iris share, biggest for small eyes), plus `browMorphs` (RaiseBrows 0 → 0.03, LowerBrows settle 0.12 → 0 with Size); the **reference feel pack** (landmine 20, last entry): `headPaintChunk` inks Head_0's lash and brow **cards** through `LASH_CARD_U` / `BROW_CARD_U` / `CARD_V` (UV windows measured on the GLB; `LASH_INK` near-black at 0.92 on the upper lash card = the liner, `BROW_INK` 0.72 toward `browColorFor(hair)` on the brow and lower-lash cards, fed by the `browColor` uniform from `tintLook`), `LASH_LENGTH` 0.8 and `OUTER_CORNER_LIFT` 0.9 on `Shape_LashLength` / `Shape_EyesOuterCornersHigh`, `LID_SHADE` 0.34, `IRIS_LOOK` (luminous collarette, radial fibres scaled by the per-frame `irisDetail` uniform from `irisDetailFor`, limbal ring 0.70 → 1.0 at 22% mixed 0.85, glow 0.14); `CATCHLIGHT` + `viewSpaceEyeCentres` / `updateEyeCentres` (the highlight is a ≤9° spot on the analytic sphere normal from the per-frame `eyeCentreL/R` view-space uniforms, 90% white plus `emit` 0.6 unlit, aimed along `dir` (0.22, −0.02, 1.0) — level with the view axis, because both cameras sit below the eyes and #36's (0.20, 0.16, 1.0) landed under the upper lid); the iris material's `polygonOffsetFactor` is **0** (constant `units` −4 only — a slope factor let the eyeball rim through the lids, see landmine 20); `MAX_GAZE_ANGLE` + `aimBoneAt` / `aimEyes` (every frame, both `eyeRoot_l/r` bones turn their local +Z onto `camera.position`; the lid margins are skinned 1.0 to those bones, so the lids turn with the gaze). `window.phViewerState()` (`.eyes` = lid weight + per-eye position / forward / `offCameraDeg` / `screen` = canvas px + px-per-metre at the eye's depth) and `window.phViewerRig` are read-only probes for tests. Metro cache-bust query lives in `AvatarEngineHost.tsx` (`?v=bozo36`); bump it with every viewer edit |
| `scripts/check-avatar-viewer.js` | Renders the real viewer in headless Chrome and asserts figure/hands in frame, eyeballs in the head, both irises aimed at the camera and converged, upper lid lowered, hair on the head, one master rig; writes screenshots plus a 4× `*-eyes.png` close-up per look. A second **pixel pass** (hoodie preset, Size 0 / 0.5 / 1, Outfit full-body and Eyes bust cameras, 3× CSS size, the rAF loop parked so the idle sway cannot move the eye between probe and capture) reads the eye pixels back and asserts iris exposure and sclera share per Size, the lid off the pupil, **full = bust** within noise, that the default eye is **not a pinprick** at either camera (analytic iris ≥ 7.4 CSS px across in full and ≥ 18.7 px in bust — above the 0.82 pass so a slide back fails; the counted opening ≥ 36 CSS px² in full only guards the 0.58 / 0.65 sizes because a 10 × 6 px blob moves ±2 px² with the parked idle-sway phase), and that the **reference feel** is on the render (bust, Size 0.5: inked lash card ≥ 0.05 r² of near-black outside the iris disc — 0 without the ink, 0.09–0.12 with it at every Size, camera and sway phase; limbal ring / mid-iris luminance ≤ 0.54; glint share of the iris disc 0.5–6%; every look: `Shape_LashLength` ≥ 0.7 and `Shape_EyesOuterCornersHigh` in 0.5–1 read back from the morph influences). The iris-exposure band and the full = bust parity are judged on the **liner-excluded** exposure (`irisExposureNoInk`) because the pixel classifier counts near-black card pixels as iris; it also writes the whole frame per Size and camera (`pixels-outfit2-<view>-size<n>-frame.png`) for review collages. `node scripts/check-avatar-viewer.js --out /tmp/ph-avatar-check` |
