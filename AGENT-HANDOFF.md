...
| `src/screens/call/status.ts` | `CallPhase`, `callStatusLabel` (`Calling Kevin` / `Connected` / `Listening…` / `Thinking…` / `Kevin is speaking`), `holdButtonLabel`, `NOTHING_HEARD_COPY` |
...
| `src/screens/call/video-stage.tsx` | Video mode. Top row = captions beside the camera PiP; below it the companion fills a rounded stage: `AvatarPreview` (`viewMode="bust"`) when `face.look`, else `Image` of `face.source`. The PiP sits *above* the stage, never inside it, because the 3D avatar is a WKWebView the engine host floats over the whole app at the stage's rect |
| `src/screens/call/camera-preview.tsx` | `CameraPreview`: the native front camera, or copy — `CAMERA_UNAVAILABLE_COPY` when the binary has no view manager, `CAMERA_DENIED_COPY` + Open Settings on `denied`, the native message on `unavailable` |
...
| `ios/AppFrontend/PHNative.mm` | `PHCameraPreviewManager` / `PHCameraPreviewView` (`RCT_EXPORT_MODULE(PHCameraPreview)`, props `position`, `onStatusChange`): `AVCaptureSession` with the front wide camera only, `automaticallyConfiguresApplicationAudioSession = NO`, starts in `didMoveToWindow`, stops when detached. `ensurePlaybackAudioSession` runs before every `speak` (landmine 22). `NSCameraUsageDescription` is in both Info.plists (`__tests__/ios-privacy-plist.test.ts`) |

`__tests__/voice-video-call.test.tsx` drives both call screens with speech / Ark / TTS mocked at the service edge; `__tests__/camera-preview-gate.test.tsx` locks the UIManager gate.
...
| Voice / video call | Real push-to-talk conversation (`src/screens/call/`): hold → iOS Speech → Ark → `AVSpeechSynthesizer`, spoken turns written to the Message thread / Love transcript, video mode = this person's face (3D bust or portrait) + front-camera PiP. Same missing-key copy as the chats. **No WebRTC** (nothing to call; the companion runs on the phone). Jest-verified only; not device-verified as of this entry. |
...
22. **A call is one shared `AVAudioSession` plus one WKWebView; keep every step off each other's toes.** (a) PHNative's speech recognizer sets the shared session to `Record` / `Measurement` and `teardownVoiceEngine` only deactivates it — the category stays `Record`, which has no output route, so the very next `AVSpeechSynthesizer` utterance is silent. `speak` now calls `ensurePlaybackAudioSession` (move `Record` → `Playback` + `DuckOthers`, `setActive:YES`) before speaking; do not route the reply through anything that skips `speak`. (b) That is also why the call is push-to-talk and not VAD: a recognizer that listens while the synthesizer speaks would need the session in `PlayAndRecord` with echo cancellation and would hear the companion; hold → release → speak is sequential on purpose. (c) The front-camera PiP is a separate `AVCaptureSession` with video input only and `automaticallyConfiguresApplicationAudioSession = NO`; never add an audio input to it. (d) `requireNativeComponent("PHCameraPreview")` on a binary that never registered the view (Android, an IPA built before the view manager) throws at render → RCTFatal in Release (landmine 11); `src/native/camera-preview.ts` asks `UIManager.getViewManagerConfig` first and the screen shows copy instead. (e) The 3D avatar on the video stage is the app-wide engine WebView floated at the stage's measured rect, so the PiP and the controls live outside that rect (`VideoStage` puts the PiP in a row above the stage) or they render underneath the avatar. `__tests__/voice-video-call.test.tsx` + `__tests__/camera-preview-gate.test.tsx` lock the JS side; the native side has only been read, not built — the first TestFlight of this needs the Xcode build log if it fails to compile.
